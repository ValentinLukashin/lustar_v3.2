...
namespace nlControls
{
    ...
    /// Класс 'crlAreaGrid'
    /// </summary>
    /// <remarks>Область для правки табличных данных</remarks>
    public class crlAreaGrid : crlArea
    {
        #region = МЕТОДЫ
        ...
        private void mButtonColumns_eMouseClickRight(object sender, EventArgs e)
        {
            ...
            ArrayList vColumnsIndexes = (FindForm() as crlForm).__oFileIni.__mParametersListByMaskInput(Name, "Column_");
            int vColumnIndexDefault = 0;
            foreach (string vColumn in vColumnsIndexes)
            {
                (FindForm() as crlForm).__oFileIni.__mParameterClear(Name, vColumn);
        ...
        {
            set { _cButtonColumns.ToolTipText = crlApplication.__oTunes.__mTranslate(value); }
        }
